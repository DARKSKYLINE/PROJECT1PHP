...
<?php
    include "connect.php";
    $sql = "SELECT * FROM tbl_smartfarm_iot WHERE id=$_GET[id]";
...
?>
    <div class="container">
        <div class="col-8">
            <h1 class="text-center"> แก้ไขข้อมูล smartfarm </h1>
            <form action="update_smartfarm.php" method="POST" enctype="multipart/form-data">
                ...
                ความชื้น : <input type="text" name="humid"class="form-control" value="<?php echo $row[humid]; ?>"/>
                <input type="submit" value="บันทึกข้อมูล แก้ไข" class="btn btn-primary" >
            </form>
        </div>
    </div>
</body>
